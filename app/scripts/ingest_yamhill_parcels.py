"""
Ingest Yamhill County taxlot parcels into parcels_yamhill.

Source: services.arcgis.com/uUvqNMGPm7axC2dD/ArcGIS/rest/services/Yamhill_County_Landowners/FeatureServer/0
        (Statewide ArcGIS Online org, 2025 Yamhill landowners layer, 42,852 parcels).

Strategy:
  Paginated REST query with outSR=4326 + f=geojson so the server reprojects from
  EPSG:2992 (Oregon Lambert ft NAD83/HARN) to WGS84. Idempotent: ON CONFLICT
  (objectid) DO UPDATE refreshes existing rows. Server maxRecordCount = 2000.

Run inside the API container:
  docker compose exec -T api python -m scripts.ingest_yamhill_parcels
"""
import os
import json
import time
...
PARCEL_URL = (
    ...
    "Yamhill_County_Landowners/FeatureServer/0/query"
)
USER_AGENT = "SKOpi-TERRA/1.0 (+https://skopi.io)"
PAGE_SIZE = 2000   # server max for this FeatureServer
BATCH_SIZE = 500
HTTP_TIMEOUT = 90
HTTP_RETRIES = 3
HTTP_RETRY_DELAY = 5
...
def ingest(conn) -> int:
    ...
    while True:
        ...
        print(f"[parcels_yamhill] page {page_num} offset={offset} got {len(feats)} | total: {total} | skipped: {skipped} | elapsed: {elapsed:.1f}s")
    ...
    elapsed = time.time() - t0
    print(f"[parcels_yamhill] DONE: {total} ingested, {skipped} skipped, {elapsed:.1f}s")
    return total
