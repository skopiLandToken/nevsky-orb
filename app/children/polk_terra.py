...
def fetch_polk_assessment(taxlot: str, force_refresh: bool = False) -> dict:
    ...
    return {
        ...
        "in_flight": [
            "Real-market valuation (RMV land/improvement/total) — Polk's public MapServer exposes only assessed values. RMV chain requires a separate assessor portal lookup that isn't currently published.",
            "Building details (year built, sqft, bedrooms, baths) — not in the public source; queued for a per-account scraper once Polk republishes the direct portal page.",
            "Zoning — Polk publishes zoning on a separate MapServer not yet ingested into a polk_zoning table. Queued behind L1+L2 ship.",
            "Polk's www.polkcountyor.gov/assessor and /clerk pages return 404 as of 2026-05-28; the only stable Polk public-facing per-parcel surface is the maps.co.polk.or.us/pcmaps ArcGIS web app. Deep links route there until Polk republishes the direct pages.",
        ],
...
def fetch_polk_records(taxlot: str, force_refresh: bool = False) -> dict:
    ...
    return {
        ...
        "deep_links": {
            "polk_maps": _POLK_MAPS_PORTAL,
            "clerk_home": _POLK_CLERK_HOMEPAGE,
            "assessor_home": _POLK_ASSESSOR_HOMEPAGE,
        },
        "in_flight": [
            "Multi-instrument deed chain — L1 carries only the most-recent instrument. Polk Clerk's public recordings portal URL is not yet located (www.polkcountyor.gov/clerk returns 404 as of 2026-05-28); surfacing the maps portal + clerk landing pointers until a working clerk URL is republished.",
